...
const LiffLogin = () => {
  const { isLoggedIn, profile, loading, error, login } = useLiff()
  const navigate = useNavigate()

  useEffect(() => {
    // ログイン済みの場合、シフト入力画面にリダイレクト
    if (isLoggedIn && profile) {
      navigate('/staff/shift-input')
    }
        ...
        <CardContent className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              LINEアカウントでログインして、シフト希望を登録できます
            </p>
          </div>
          <Button
            onClick={login}
            className="w-full h-12 bg-[#06C755] hover:bg-[#05B04B] text-white text-lg font-medium"
          >
