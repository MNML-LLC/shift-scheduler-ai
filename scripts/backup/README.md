...
## DB Backup to SharePoint

`db-backup-to-sharepoint.yml.template` は SharePoint 承認待ちの未稼働ワークフロー。承認後、以下の手順で有効化する:

1. テンプレ先頭のコメントブロックを参照し、Azure AD アプリ登録と GitHub Secrets 設定を完了させる
2. ファイルを `.github/workflows/db-backup-to-sharepoint.yml` に移動
3. ファイル先頭のコメントブロックを削除し、`name:` 以降のコメントアウトを解除して有効化
